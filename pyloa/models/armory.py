...
class ArmoryProfile(BaseModel):
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'ArmoryProfile':
        ...
        decorations = Decoration.from_dict(decorations_data) if decorations_data else None
...
class ArmoryEngraving(BaseModel):
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'ArmoryEngraving':
        ...
        return cls(
            ...
            ark_passive_effects=[ArkPassiveEffect.from_dict(e) for e in (data.get('ArkPassiveEffects') or [])]
        )
...
class ArmoryGem(BaseModel):
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'ArmoryGem':
        ...
        return cls(
            gems=[Gem.from_dict(g) for g in (data.get('Gems') or [])],
            effects=effects
        )
...
class Colosseum(BaseModel):
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Colosseum':
        ...
        return cls(
            ...
            competitive=AggregationTeamDeathMatchRank.from_dict(competitive_data) if competitive_data else None,
            team_deathmatch=AggregationTeamDeathMatch.from_dict(team_dm_data) if team_dm_data else None,
            team_elimination=AggregationElimination.from_dict(team_el_data) if team_el_data else None,
            co_op_battle=Aggregation.from_dict(coop_data) if coop_data else None,
            one_deathmatch=AggregationOneDeathmatch.from_dict(one_dm_data) if one_dm_data else None,
            one_deathmatch_rank=AggregationOneDeathmatch.from_dict(one_dm_rank_data) if one_dm_rank_data else None
        )
...
class Collectible(BaseModel):
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Collectible':
        ...
        return cls(
            ...
            collectible_points=[CollectiblePoint.from_dict(p) for p in (data.get('CollectiblePoints') or [])]
        )
...
class ArkPassive(BaseModel):
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'ArkPassive':
        ...
        return cls(
            ...
            effects=[ArkPassiveEffectSkill.from_dict(e) for e in (data.get('Effects') or [])]
        )
...
class ArmoryTotal(BaseModel):
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'ArmoryTotal':
        ...
        equipment = [ArmoryEquipment.from_dict(item) for item in (data.get('ArmoryEquipment') or [])]
        avatars = [ArmoryAvatar.from_dict(item) for item in (data.get('ArmoryAvatars') or [])]
        skills = [ArmorySkill.from_dict(item) for item in (data.get('ArmorySkills') or [])]
        
        engraving_data = data.get('ArmoryEngraving')
        armory_engraving = ArmoryEngraving.from_dict(engraving_data) if engraving_data else None
        ...
        colosseum_info = ColosseumInfo.from_dict(colosseum_data) if colosseum_data else None
        
        collectibles = [Collectible.from_dict(item) for item in (data.get('Collectibles') or [])]
        
        ark_passive_data = data.get('ArkPassive')
        ark_passive = ArkPassive.from_dict(ark_passive_data) if ark_passive_data else None
